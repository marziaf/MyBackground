 ...
 * class is constructed, and remains alive for command execution until the
 * appropriate function is called, which stops the MATLAB engine too.
 *
 */
public class MatlabBinderInstance implements Runnable {
	...
	/**
	 * Calls the MATLAB function "evalAsync" to evaluate "command" without need to
	 * create a separate flow of execution here. To wait for the completion of it,
	 * one should wait until "computing" is set to false. -> wait for notify or
	 * busy-waiting? Busy-Waiting, because the Future object returned is made for
	 * it.
	 * 
	 * @param command the string to evaluate as a command
	 */
	public void computeCommandAsynchronously(String command) {
		while (computing != null && isComputing()) {
		} // if another command was executed and it is still running, wait
		computing = engine.evalAsync(command);
	}

	// There is no interaction from the server to the matlab engine! All variables
	// are exchanged
	// via the server non-volatile memory
	// public void getVariableAsynchronously(String variable) {
	// while(computing != null && isComputing()) {} //if another command was
	// executed and it is still running, wait
	// computing = engine.getVariableAsync(variable);
	//
	// }

	/**
	 * The run() method of this runnable class. This method is used to keep the
	 * engine alive, to avoid loosing the data on every call
	 */
	@Override
	public void run() {
		...
		try {
			...
			e.printStackTrace();
			System.out.println("engine failed closing");
		}
		to_stop = false;
	...
	/**
	 * Method that manages to keep the engine alive and respond to notifies on
	 * whether or not to start the engine (just here to keep run() clean)
	 */
		...
		while (!to_stop) { // to_stop and computing are only read, no need for synchronization
			...
			if (!to_start || isComputing()) {
				try {
					...
					System.out.println("Interruption waiting for engine start");
					e.printStackTrace();
				}
			} else {
				try {
	...
	/**
	 * This method is used to stop this thread, by signaling with a flag that the
	 * Main Thread wants to stop this instance. From the MT, call join() to wait for
	 * this to happen.
	 */
	public void close() {
		synchronized (engineOnOffLock) {
	...
	/**
	 * Sets the flag to start the matlab engine on the parallel task run(), should
	 * not be called twice.
	 * 
	...
	/**
	 * Returns the information on whether or not MATLAB is currently evaluating a
	 * command it was sent. Used in a busy-waiting scheme when executing a certain
	 * task from the main thread
	 * 
	 * @return true if the matlab engine is currently evaluating a command sent by
